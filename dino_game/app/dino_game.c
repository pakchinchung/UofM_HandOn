    ...
    for (i = 0; i < OBSTACLE_COUNT; i++)
    {
        if (obstacles[i].active)
        {
            ...
            else if (GAME_HasCollided(&obstacles[i]))
            {
                ...
                hudForceRedraw = true;
                UART_Printf("Game over. Score %u, best %u\r\n", gameScore, gameHighScore);
                return;
            }
            else
            {
...
void GAME_Initialize(void)
{
    ...
    UART_Printf("Dino ready. GP6 start, GP5 jump, GP7 menu. Pot sets speed.\r\n");
    UART_Printf("Jump: gravity %u, apex target %u px -> impulse %d, "
